

const ImpactSummary = () => {
  return (
    <div className=" w-[100%] py-12 px-4 sm:px-[2rem] lg:px-[2rem]">
      <div className="  text-center">
        <h2 className="text-4xl lg:text-7xl font-normal text-black">OUR <span className="text-green">IMPACT</span></h2>
        <div className="mt-16 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            <h1 className="text-8xl font-normal text-green">1+</h1>
            <h1 className="text-4xl font-medium text-green">LAC</h1>
            <p className="mt-2 text-black text-wrap">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
          </div>
          <div>
            <h1 className="text-8xl font-normal text-green">1000+</h1>
            <h1 className="text-4xl font-medium text-green">VILLAGES</h1>
            <p className="mt-2 text-black text-wrap">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
          </div>
          <div>
            <h1 className="text-8xl font-normal text-green">500+</h1>
            <h1 className="text-4xl font-medium text-green">CAMPAIGNS</h1>
            <p className="mt-2 text-black text-wrap">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
          </div>
          <div>
            <h1 className="text-8xl font-normal text-green">25+</h1>
            <h1 className="text-4xl font-medium text-green">STATES</h1>
            <p className="mt-2 text-black text-wrap">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
          </div>
        </div>
      </div>
    </div>
  );
}

export default ImpactSummary;
